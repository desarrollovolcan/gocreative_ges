<div class="card">
    ...
    <div class="card-body">
        <form method="post" action="index.php?route=accounting/chart/update">
            <input type="hidden" name="csrf_token" value="<?php echo csrf_token(); ?>">
            <input type="hidden" name="id" value="<?php echo (int)$account['id']; ?>">
            <div class="row g-3">
                <div class="col-md-3">
                    <label class="form-label">Código</label>
                    <input type="text" name="code" class="form-control" value="<?php echo e($account['code']); ?>" required>
                </div>
                <div class="col-md-5">
                    ...
                    <select name="parent_id" class="form-select">
                        ...
                        <?php foreach ($accounts as $parentAccount): ?>
                            <?php if ((int)$parentAccount['id'] === (int)$account['id']): ?>
                                <?php continue; ?>
                            <?php endif; ?>
                            <option value="<?php echo (int)$parentAccount['id']; ?>" <?php echo (int)($account['parent_id'] ?? 0) === (int)$parentAccount['id'] ? 'selected' : ''; ?>>
